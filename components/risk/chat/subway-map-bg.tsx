...
function generateScene(w: number, h: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    points.push({
      ...
      baseAlpha: rand(0.015, 0.04) * (0.6 + centralness * 0.6),
    ...
    for (let j = i + 1; j < points.length; j++) {
      ...
      if (d < CONNECTION_DIST && Math.random() < 0.3) {
        connections.push({
          a: i,
          b: j,
          alpha: Math.max(0.005, 0.025 * (1 - d / CONNECTION_DIST)),
        });
      }
    }
  }
    ...
    const animate = () => {
      ...
      for (let gx = -half; gx <= half; gx += GRID_SPACING) {
        ...
        if (p1 && p2) {
          ...
          ctx.strokeStyle = `rgba(100,116,139,${0.008 + hover * 0.02})`;
          ctx.lineWidth = 0.5;
          ctx.stroke();
        }
      }
      ...
      fogTop.addColorStop(0, "rgba(255,255,255,0.6)");
      ...
      fogBottom.addColorStop(1, "rgba(255,255,255,0.4)");
...
function drawStatic(ctx: CanvasRenderingContext2D, scene: Scene, w: number, h: number) {
  ...
  for (let gx = -half; gx <= half; gx += GRID_SPACING) {
    ...
    if (p1 && p2) {
      ...
      ctx.strokeStyle = "rgba(100,116,139,0.015)"; ctx.lineWidth = 0.5; ctx.stroke();
    }
  }
  for (let gz = -half * 0.65; gz <= half * 0.45; gz += GRID_SPACING) {
    const p1 = project(cx - half, 0, gz, cx, cy);
