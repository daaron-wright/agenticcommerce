...
export function CommercialReviewCard() {
  ...
  return (
    <Card className="border-stone-200 shadow-none overflow-hidden">
      <CardHeader className="border-b bg-stone-50/70 pb-3">
          ...
          <div className="grid gap-2 lg:grid-cols-2">
            {WEEKLY_COMMERCIAL_REVIEW.keyTakeaways.map((item) => (
              ...
              >
                ...
                <p className="text-xs text-stone-700 leading-relaxed">{item.narrative}</p>
                <p className="text-[11px] font-medium text-stone-800">{item.metric}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-2">
          ...
          <div className="overflow-x-auto rounded-lg border border-stone-200">
            <table className="w-full border-collapse text-xs min-w-[560px]">
              <thead>
                <tr className="bg-stone-50 text-muted-foreground">
              ...
              <tbody>
                {WEEKLY_COMMERCIAL_REVIEW.kpis.map((row) => (
                  <tr key={row.label} className="border-t border-stone-200">
                    <td className="px-3 py-2 font-medium text-stone-800">{row.label}</td>
              ...
              <table className="w-full border-collapse text-xs min-w-[520px]">
                ...
                <tbody>
                  {WEEKLY_COMMERCIAL_REVIEW.marketBreakdown.map((row) => (
                    <tr key={row.market} className="border-t border-stone-200">
                      <td className="px-3 py-2">
                        <div className="font-medium text-stone-800">{row.market}</div>
              ...
              <table className="w-full border-collapse text-xs min-w-[560px]">
                ...
                <tbody>
                  {WEEKLY_COMMERCIAL_REVIEW.paidChannelPerformance.map((row) => (
                    <tr key={row.channel} className="border-t border-stone-200">
                      <td className="px-3 py-2">
                        <div className="font-medium text-stone-800">{row.channel}</div>
        ...
        <section className="space-y-2">
          ...
          <ul className="space-y-2">
            {WEEKLY_COMMERCIAL_REVIEW.discussionFlags.map((flag) => (
              <li key={flag} className="rounded-lg border border-stone-200 bg-stone-50 px-3 py-2 text-xs text-stone-700 leading-relaxed">
                {flag}
              </li>
            ))}
          </ul>
        ...
        <section className="space-y-2">
          ...
          <div className="space-y-2">
            {WEEKLY_COMMERCIAL_REVIEW.recommendations.map((recommendation) => (
              <div key={recommendation.id} className="rounded-lg border border-stone-200 bg-stone-50 p-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-semibold text-stone-800">{recommendation.title}</p>
                  <span
                    className={cn(
                ...
                <p className="mt-1 text-[11px] text-stone-700">{recommendation.action}</p>
                ...
                <div className="mt-2 flex items-center gap-2">
                  <Link
                    ...
                    className="inline-flex items-center rounded-md bg-stone-700 px-2.5 py-1.5 text-[11px] font-medium text-white hover:bg-stone-800"
                  >
