      ...
      <Card className="rounded-[24px] border border-slate-200 bg-white/95 shadow-none">
        <CardContent className="flex min-h-[280px] items-center justify-center p-6 text-center">
          <div className="max-w-sm space-y-2">
            <p className="text-sm font-semibold text-slate-900">Select a node to inspect it</p>
  ...
  return (
    <Card className="rounded-[24px] border border-slate-200 bg-white/95 shadow-none">
      <CardHeader className="space-y-3 pb-4">
        ...
        <div>
          <CardTitle className="text-lg font-semibold text-slate-900">{node.label}</CardTitle>
      ...
      <CardContent className="space-y-5">
          ...
          <div className="mt-3 space-y-2">
            {node.attributes.map((attribute) => (
              <div
                ...
                className="flex items-start justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 px-3 py-2.5"
              >
                <span className="text-xs text-slate-500">{attribute.label}</span>
                <span className="text-xs font-semibold text-slate-800">{attribute.value}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            <div className="mt-3 space-y-2">
              {node.suggestedActions.map((action) => (
                <Link
                  ...
                  href={action.href}
                  className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-700 transition hover:border-slate-300 hover:bg-slate-50"
                >
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
          ...
          <Button asChild className="mt-3 h-9 rounded-full bg-slate-900 px-4 text-xs hover:bg-slate-950">
